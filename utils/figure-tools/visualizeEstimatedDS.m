...
elseif M == 3
    ...
    if isfield(ds_plot_options,'save_path') && ~isempty(ds_plot_options.save_path)
        % Make sure the figure was rendered
        drawnow;
        % Use PDF output at 300 dpi
        set(fig1,'PaperPositionMode','auto');
        % print(fig1, ds_plot_options.save_path, '-dpdf','-r300');
        % Or, if you prefer vector graphics in newer MATLAB:
        exportgraphics(fig1, ds_plot_options.save_path, 'ContentType','vector');
    end
