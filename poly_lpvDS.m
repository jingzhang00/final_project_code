...
% K = length(Priors);
% sos_options.degree_f = 3;
% sos_options.degree_V = 2;
% sos_options.epsilon = 1e-6;
% sos_options.maxIter = 10;
% piecewise_ds = cell(1,K);
% 
% for i = 1:K
%     idx_i = find(est_labels == i);
%     Xi_i     = Xi_ref(:,   idx_i);
%     Xi_dot_i = Xi_dot_ref(:,   idx_i);
% 
%     [poly_f, poly_V] = optimize_polynomial_lpv_ds(Xi_i, Xi_dot_i, sos_options);
%     piecewise_ds{i}.f = poly_f;
%     piecewise_ds{i}.V = poly_V;
%     piecewise_ds{i}.Mu = Mu(:,i);
%     piecewise_ds{i}.Sigma = Sigma(:,:,i);
%     piecewise_ds{i}.Prior = Priors(i);
% end
% 
% ds_lpv = @(x) evaluate_piecewise_ds(x, piecewise_ds);

N = size(Xi_ref,2);
K = numel(ds_gmm.Priors);
x_star = att;

% posterior weights: R(i,n) = π_i * N(x_n|μ_i,Σ_i)
logR = zeros(K,N);
for i=1:K
  logR(i,:) = log(ds_gmm.Priors(i)) + ...
    log( mvnpdf(Xi_ref', ds_gmm.Mu(:,i)', ds_gmm.Sigma(:,:,i)) )';
end
logR = logR - max(logR,[],1);
R    = exp(logR);
R    = R ./ sum(R,1);   % K×N
...
for i=1:K
  w = R(i,:)';          % N×1
  if sum(w) < 1e-2
    warning('Region %d weight too small, skipping', i);
    continue;
  end

  % min ||W^(1/2)*(A*(X-x*) - Xdot)||_F^2
  W2 = diag(sqrt(w));
  Xc = Xi_ref - x_star;    % 2×N
  Y  = Xi_dot_ref;         % 2×N

  cvx_begin sdp quiet
    variable A(2,2)
    minimize( norm( W2*(A*Xc - Y)', 'fro' ) ) 
    subject to
      A'*P_opt + P_opt*A <= -eye(2)*eps_LMI;
  cvx_end

  if ~strcmp(cvx_status, 'Solved')
    warning('A_%d not solved, using zero matrix', i);
    A = zeros(2,2);
  end
  A_cell{i} = A;
end

ds_lpv = @(X) eval_lpv_ds(X, ds_gmm, A_cell, x_star);
tEnd = cputime - tStart;
%% 
...
% 
% 
% limits_ds = [min(Xi_ref(1,:))-pad, max(Xi_ref(1,:))+pad, ...
%                min(Xi_ref(2,:))-pad, max(Xi_ref(2,:))+pad];
% 
% lmds_data = simulate_lmds_gp_learned(Target, limits_ds, fig1, ls_gpr, A, {Data});
